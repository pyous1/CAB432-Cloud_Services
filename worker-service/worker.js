...
async function handleConvertImages(job) {
  ...
  const key = `results/${job.username || "public-user"}-images-${Date.now()}.pdf`;
...
async function handleMergePDFs(job) {
  ...
  const key = `results/${job.username || "public-user"}-merged-${Date.now()}.pdf`;
  await uploadResult(Buffer.from(pdfBytes), key);
  return key;
}
...
async function handleWatermark(job) {
  ...
  const key = `results/${job.username || "public-user"}-watermarked-${Date.now()}.pdf`;
...
async function handleLatex(job) {
  ...
  const key = `results/${job.username || "public-user"}-latex-${Date.now()}.pdf`;
...
async function handleExternalFetch(job) {
  ...
  const key = `results/${job.username || "public-user"}-external-${Date.now()}.pdf`;
...
async function processJob(job) {
  try {
    await addHistory(job.username || "public-user", "worker: IN_PROGRESS", {
      jobId: job.jobId,
      type: job.type,
    });

    console.log(`🔧 Processing ${job.type} for ${job.username || "public-user"} (jobId=${job.jobId || "n/a"})`);
    ...
    if (resultKey) {
      await addHistory(job.username || "public-user", "worker: COMPLETED", {
        jobId: job.jobId,
        type: job.type,
        resultKey,
      });
    ...
    await addHistory(job.username || "public-user", "worker: FAILED", {
      jobId: job.jobId,
      type: job.type,
      error: err.message,
    });
  ...
  try {
    const entry = {
      ...
      Detail: JSON.stringify({
        ...
        username: job.username || "public-user",
        type: job.type,
        resultKey,
    ...
    console.log("📨 EventBridge notification sent:", JSON.stringify(resp));
  } catch (err) {
    console.error("❌ Failed to send EventBridge event:", err.message);
  }
}
